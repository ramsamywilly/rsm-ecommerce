...
const UserReviews = () => {
  ...
  if(error) return <div>Échec du chargement des avis!</div>
  ...
  return (
  <div className= 'py-6'>
  		<h2 className= 'text-xl font-semibold mb-8'>Vos avis donnés</h2>
  		<div className= 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mt-8 gap-6'>
  			{
  				reviews && reviews.map((review, index) => (
  						<div key={index} className= 'bg-white shadow-md rounded-lg p-4 border-gray-200 cursor-pointer hover:scale-105 transition-all duration-200'>
  							<p className= 'italic text-lg text-gray-800 font-medium mb-5'>Notation : {review?.rating} étoiles</p>
  							<p className= 'mb-2'><strong>Commentaire : </strong>{review?.comment}</p>
  							<p className= 'text-sm text-gray-500'><strong>Identifient produit : </strong>{review?.productId}</p>
